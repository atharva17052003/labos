# include <stdio.h>
	...
	void insert(struct node **front, struct node **rear , int value, int priority)
	{
		struct node *temp,*temp1;
 		temp=(struct node *)malloc(sizeof(struct node)); /* creates new node using data value
																		passed as  parameter */
      if(temp==NULL)
      {
      ...
      if(*rear == NULL)   /* This is the first node */
        {
            ...
            if( (*rear)->priority > priority)   /* the element to be inserted has
              ...
              {
                temp1 = *front;
                while((temp1->link)->priority >= priority) /* find the position and insert the new element */
                	temp1=temp1->link;
