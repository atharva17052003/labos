...
struct dnode *temp;
 ...
 if(p==NULL)
   {
      p=(struct dnode *)malloc(sizeof(struct dnode)); /* creates new node data value 
   passed as  parameter */
    ...
    {
       temp =  (struct dnode *)malloc(sizeof(struct dnode)); /* creates new node using 
...
/* a function which inserts a newly created node after the specified node in a doubly
       linked list */ 
...
void main()
{
      ...
      printfor ( start );
      printf("enter the number of node which is to be deleted\n");
      scanf("%d",&n);
